...
def calc_subnet(
    ip: str = Query(..., description="IP 地址，如 192.168.1.10", example="192.168.1.10"),
    mask: str = Query(
        ...,
...
def split_subnet(
    network: str = Query(..., description="原网络地址", example="192.168.1.0"),
    prefix: int = Query(..., ge=0, le=32, description="原前缀长度", example=24),
    new_prefix: int = Query(..., ge=0, le=32, description="新前缀长度", example=26),
) -> dict:
...
def ip_range_to_cidr(
    start: str = Query(..., description="起始 IP", example="192.168.1.0"),
    end: str = Query(..., description="结束 IP", example="192.168.1.127"),
) -> dict:
